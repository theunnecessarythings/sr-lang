...
module Func = struct
  ...
  let rust_binding_args t ~self =
    List.map t.args ~f:(fun arg ->
      ...
      | _ -> name)
    |> String.concat ~sep:",\n                "
  (* SR (.sr) generation helpers *)
  (* Note: SR helpers are implemented outside of Func and used directly. *)
end
    ...
    then
      ...
      |> Option.bind ~f:(fun (returns, kind) ->
           try
             ...
             in
             (* Touch default_value to silence unused field warnings in strict builds. *)
             List.iter args ~f:(fun (a: Func.arg) -> ignore a.default_value);
...
(* SR helpers and writers *)
let sanitize_arg_name an =
  if String.(=) an "tensor" then "tensor_"
  else if String.(=) an "any" then "r#any"
  else if String.(=) an "complex" then "r#complex"
  else an

let sr_decl_and_names_for_arg ~qualify (arg : Func.arg) =
  let an = sanitize_arg_name arg.arg_name in
  let q ?(ptr=false) typ =
    let pref t =
      if String.is_empty qualify then t else Printf.sprintf "%s.%s" qualify t
    in
    match typ with
    | "Tensor" -> if ptr then Printf.sprintf "*%s" (pref "Tensor") else pref "Tensor"
    | "Scalar" -> if ptr then Printf.sprintf "*%s" (pref "Scalar") else pref "Scalar"
    | other -> if ptr then Printf.sprintf "*%s" other else other
  in
  match arg.arg_type with
  | IntList | IntListOption -> [ Printf.sprintf "%s_data: *i64" an; Printf.sprintf "%s_len: i32" an ], [ an ^ "_data"; an ^ "_len" ]
  | DoubleList -> [ Printf.sprintf "%s_data: *f64" an; Printf.sprintf "%s_len: i32" an ], [ an ^ "_data"; an ^ "_len" ]
  | TensorOptList | TensorList ->
    [ Printf.sprintf "%s_data: %s" an (q ~ptr:true "Tensor"); Printf.sprintf "%s_len: i32" an ], [ an ^ "_data"; an ^ "_len" ]
  | TensorOptions -> [ Printf.sprintf "%s_kind: i32" an; Printf.sprintf "%s_device: i32" an ], [ an ^ "_kind"; an ^ "_device" ]
  | String -> [ Printf.sprintf "%s_ptr: *u8" an; Printf.sprintf "%s_len: i32" an ], [ an ^ "_ptr"; an ^ "_len" ]
  | Int64Option -> [ Printf.sprintf "%s_v: i64" an; Printf.sprintf "%s_null: u8" an ], [ an ^ "_v"; an ^ "_null" ]
  | DoubleOption -> [ Printf.sprintf "%s_v: f64" an; Printf.sprintf "%s_null: u8" an ], [ an ^ "_v"; an ^ "_null" ]
  | Bool -> [ Printf.sprintf "%s: bool" an ], [ an ]
  | Int64 -> [ Printf.sprintf "%s: i64" an ], [ an ]
  | Double -> [ Printf.sprintf "%s: f64" an ], [ an ]
  | Tensor -> [ Printf.sprintf "%s: %s" an (q "Tensor") ], [ an ]
  | TensorOption -> [ Printf.sprintf "%s: %s" an (q "Tensor") ], [ an ]
  | Scalar -> [ Printf.sprintf "%s: %s" an (q "Scalar") ], [ an ]
  | ScalarType -> [ Printf.sprintf "%s: i32" an ], [ an ]
  | ScalarTypeOption -> [ Printf.sprintf "%s: i32" an ], [ an ]
  | Device -> [ Printf.sprintf "%s: i32" an ], [ an ]
  | Layout -> [ Printf.sprintf "%s: i8" an ], [ an ]
  | LayoutOption -> [ Printf.sprintf "%s: i8" an ], [ an ]

let write_sr_bindings funcs filename =
  Out_channel.with_file filename ~f:(fun out_sr ->
    let ps s = p out_sr s in
    ps "package torch_bindings";
    ps "";
    ps "// THIS FILE IS AUTOMATICALLY GENERATED, DO NOT EDIT BY HAND!";
    ps "";
    ps "Tensor :: *void";
    ps "Scalar :: *void";
    ps "";
    ps "free :: extern proc(ptr: *void)";
    ps "";
    ps "// --- Bindings ---";
    ps "";
    Map.iteri funcs ~f:(fun ~key:exported_name ~data:func ->
      let arg_decls, _ =
        List.map func.Func.args ~f:(sr_decl_and_names_for_arg ~qualify:"")
        |> List.unzip
      in
      let sr_args = arg_decls |> List.concat |> String.concat ~sep:", " in
      let decl =
        match func.returns with
        | `nothing -> Printf.sprintf "atg_%s :: extern proc(%s)" exported_name sr_args
        | `fixed _ ->
          let out = "out__: *Tensor" in
          let args = if String.is_empty sr_args then out else Printf.sprintf "%s, %s" out sr_args in
          Printf.sprintf "atg_%s :: extern proc(%s)" exported_name args
        | `dynamic -> Printf.sprintf "atg_%s :: extern fn(%s) *Tensor" exported_name sr_args
        | `bool -> Printf.sprintf "atg_%s :: extern fn(%s) bool" exported_name sr_args
        | `int64_t -> Printf.sprintf "atg_%s :: extern fn(%s) i64" exported_name sr_args
        | `double -> Printf.sprintf "atg_%s :: extern fn(%s) f64" exported_name sr_args
      in
      ps "%s" decl))

let write_sr_wrappers funcs filename =
  Out_channel.with_file filename ~f:(fun out_sr ->
    let ps s = p out_sr s in
    ps "package torch";
    ps "";
    ...
    ps "";
    ps "// --- Wrappers ---";
    ps "torch_bindings :: import \"torch_bindings\"";
    ps "";
    ps "Tensor :: struct { t: torch_bindings.Tensor }";
    ps "";
    Map.iteri funcs ~f:(fun ~key:exported_name ~data:func ->
      let args = func.Func.args in
      let self_idx_opt =
        match List.findi args ~f:(fun _ a -> match a.Func.arg_type with | Func.Tensor -> String.(=) a.Func.arg_name "self" | _ -> false) with
        | Some (i, a) -> Some (i, a)
        | None ->
          (match List.findi args ~f:(fun _ a -> match a.Func.arg_type with | Func.Tensor -> String.(=) a.Func.arg_name "input" | _ -> false) with
           | Some (i, a) -> Some (i, a)
           | None -> None)
      in
      let self_arg_opt, other_args =
        match self_idx_opt with
        | Some (i, a) -> Some a, (List.take args i @ List.drop args (i+1))
        | None -> None, args
      in
      let has_self = Option.is_some self_arg_opt in
      let self_decl = if has_self then Some "self: *Tensor" else None in
      let per_arg_decls_names =
        List.mapi other_args ~f:(fun _ a -> sr_decl_and_names_for_arg ~qualify:"torch_bindings" a)
      in
      let other_decls = per_arg_decls_names |> List.map ~f:fst |> List.concat in
      let param_decls =
        (match self_decl with None -> [] | Some s -> [ s ]) @ other_decls
        |> String.concat ~sep:", "
      in
      let call_names =
        List.mapi args ~f:(fun idx a ->
          match self_idx_opt with
          | Some (i, _) when i = idx -> [ "self.t" ]
          | _ -> let _, names = sr_decl_and_names_for_arg ~qualify:"torch_bindings" a in names)
        |> List.concat
        |> String.concat ~sep:", "
      in
      let method_name =
        if String.(=) exported_name "any" then "r#any"
        else if String.(=) exported_name "complex" then "r#complex"
        else exported_name
      in
      let wrapper_name =
        match func.kind with
        | `function_ -> Printf.sprintf "Tensor.%s" method_name
        | `method_ -> Printf.sprintf "Tensor.%s" method_name
      in
      let ret_type =
        match func.returns with
        | `nothing -> Some "void"
        | `fixed 1 -> Some "Tensor"
        | `fixed v ->
          Some (List.init v ~f:(fun _ -> "Tensor") |> String.concat ~sep:", " |> Printf.sprintf "(%s)")
        | `dynamic -> Some "[dyn] Tensor"
        | `bool -> Some "bool"
        | `int64_t -> Some "i64"
        | `double -> Some "f64"
      in
      let ret_sig = match ret_type with None -> "" | Some r -> Printf.sprintf " %s" r in
      ps "%s :: proc(%s)%s {" wrapper_name param_decls ret_sig;
      (match func.returns with
       | `nothing ->
         ps "    torch_bindings.atg_%s(%s)" exported_name call_names
       | `fixed ntensors ->
         ps "    outs: [%d] torch_bindings.Tensor = undefined" ntensors;
         let call_full = if String.is_empty call_names then "&outs[0]" else Printf.sprintf "&outs[0], %s" call_names in
         ps "    torch_bindings.atg_%s(%s)" exported_name call_full;
         if ntensors = 1 then ps "    return Tensor{ t: outs[0] }"
         else (
           let returns = List.init ntensors ~f:(fun i -> Printf.sprintf "Tensor{ t: outs[%d] }" i) |> String.concat ~sep:", " in
           ps "    return (%s)" returns)
       | `dynamic ->
         ps "    ptr := torch_bindings.atg_%s(%s)" exported_name call_names;
         ps "    arr: [dyn] Tensor = []";
         ps "    ptr_addr := ptr.^i64";
         ps "    i := 0";
         ps "    while true {";
         ps "        elem_addr := ptr_addr + i * 8";
         ps "        elem_ptr := elem_addr.^*(*torch_bindings.Tensor)";
         ps "        c := elem_ptr.^torch_bindings.Tensor";
         ps "        if c.^i64 == 0 { break }";
         ps "        arr.append(Tensor{ t: c })";
         ps "        i += 1";
         ps "    }";
         ps "    torch_bindings.free(ptr.^*void)";
         ps "    return arr"
       | `bool | `int64_t | `double ->
         ps "    return torch_bindings.atg_%s(%s)" exported_name call_names);
      ps "}";
      ps ""))
...
let run
  ...
  ~sr_bindings_filename
  ~sr_wrappers_filename
  =
  ...
  write_sr_bindings funcs sr_bindings_filename;
  write_sr_wrappers funcs sr_wrappers_filename

let () =
  run
    ~yaml_filename:"Declarations-v2.9.0.yaml"
    ...
    ~sr_bindings_filename:"../torch_bindings.sr"
    ~sr_wrappers_filename:"../torch.sr"
