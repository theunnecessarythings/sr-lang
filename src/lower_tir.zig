...
pub fn runAst(self: *LowerTir, ast_unit: *ast.Ast) !tir.TIR {
    ...
    try self.monomorphizer.run(self, ast_unit, &b, monomorphLowerCallback);
...
fn optLoc(a: *ast.Ast, id: anytype) tir.OptLocId {
...
fn lowerMatch(
    ...
    if (produce_value) {
        ...
        while (j < arms.len) : (j += 1) {
            ...
            var saved = List(BindingSnapshot){};
            ...
            if (!arm.guard.isNone()) {
                ...
                const guard_loc = optLoc(a, guard_id);
...
fn lowerWhile(
    ...
    if (produce_value) {
        ...
        if (row.is_pattern and !row.pattern.isNone() and !row.cond.isNone()) {
            ...
            const cond_loc = if (!row.cond.isNone()) optLoc(a, row.cond.unwrap()) else loc;
...
fn lowerFor(
    ...
    const loc = optLoc(a, id);
    const iterable_loc = optLoc(a, row.iterable);
...
const Env = struct {
    ...
    fn bind(self: *Env, gpa: std.mem.Allocator, _: *ast.Ast, name: StrId, vb: ValueBinding) !void {
