...
fn locOfCstExpr(e: *const cst.Expr) Loc {
    return switch (e.*) {
        ...
        .Function => |x| x.loc,
        .Block => |x| x.loc,
        .Comptime => |ct| switch (ct) { .Block => |blk| blk.loc, .Expr => |inner| locOfCstExpr(inner) },
        .Code => |x| x.loc,
        .Insert => |x| x.loc,
        .Mlir => |x| x.loc,
        .Call => |x| x.loc,
...
fn lowerExpr(self: *Lower, e: *const cst.Expr) anyerror!*ast.Expr {
    const out = switch (e.*) {
        ...
        .Mlir => |ml| ast.Expr{ .MlirBlock = .{ .kind = switch (ml.kind) { .Module => .Module, .Type => .Type, .Attribute => .Attribute, .Operation => .Operation }, .text = ml.text, .loc = ml.loc } },
