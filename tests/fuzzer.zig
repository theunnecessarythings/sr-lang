...
fn testParser(data: []const u8) !void {
    ...
    var parser_mod = parser.Parser.init(gpa, source0, 0, context.diags, &context);
    var tree = parser_mod.parse() catch |err| switch (err) {
        error.UnexpectedToken => {
            try std.testing.expect(context.diags.anyErrors());
            return;
        },
...
fn testLower(data: []const u8) !void {
    ...
    var parser_mod = parser.Parser.init(gpa, source0, 0, context.diags, &context);
    var tree = parser_mod.parse() catch |err| switch (err) {
        error.UnexpectedToken => return,
        error.OutOfMemory => std.debug.panic("parser OOM", .{}),
        else => std.debug.panic("parser failed: {}", .{err}),
    };
    defer tree.deinit();

    var lower_mod = try lower.Lower.init(gpa, &tree, &context, 0);
...
fn testChecker(data: []const u8) !void {
    ...
    var lower_mod = try lower.Lower.init(gpa, &c, &context, 0);
    defer lower_mod.deinit();
    var result = try (&lower_mod).run();
    defer result.deinit(gpa);
    const a = result.ast_unit;
    ...
    var chk = checker.Checker.init(gpa, &context, &pipeline);
    defer chk.deinit();
