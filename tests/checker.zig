...
test "string and char literals - success" {
    ...
    try checkProgram("sb :: br#\"raw bytes\"#", &.{});
...
test "identifiers - failures" {
    ...
    // Using a keyword without raw prefix
    try checkProgram("and :: 1", &[_]diag.DiagnosticCode{.unexpected_token});

    // Hyphen not allowed in identifier
    try checkProgram("foo-bar :: 1", &[_]diag.DiagnosticCode{.unexpected_token});

    // Invalid raw identifier forms
    try checkProgram("r# :: 1", &[_]diag.DiagnosticCode{.unexpected_token});
    try checkProgram("r#123 :: 1", &[_]diag.DiagnosticCode{.unexpected_token});
}
...
test "binary expressions - failures" {
    ...
    try checkProgram(
        ...
        \\ x: ?i32 = 1
        \\ r :: x orelse 2.0
    , &[_]diag.DiagnosticCode{.wrong_arity_in_call});
}
...
test "error handling - failures" {
    ...
    // catch handler result type mismatch
    try checkProgram(
        \\
        \\ MyErr :: error { A }
        \\ x: i32!MyErr = 1
...
test "call expressions - success" {
    ...
    // Extern call (signature borrowed from examples)
    try checkProgram(
        \\
        \\ printf :: extern proc(*void, any) i32
        \\ rr :: printf(null, \"ok\")
...
test "call expressions - failures" {
    ...
    // Unknown function
    try checkProgram("unk :: foo(1)", &[_]diag.DiagnosticCode{.undefined_identifier});
}
